...
class Node:
	...
	def __str__(self):
		return f"{self.featureIndex}: value: {self.featureValue} {[str(node) for node in self.children]} label: {self.label} "
...
class ID3:
	...
	def generateDecisionTree(self, X, y, parent):
		...
		if np.size(X) == 0:
			pluralityValue = max(set(parent), key=list(parent).count)
			...
			pluralityValue = max(set(y), key=y.count)
			node.label = pluralityValue
			return node
			...
			#Find the column index of the feature with the highest gain
			for i in range(len(X[0])):
				diff = entropy - informationNeededList[i]
				if diff > gain:
					gain = diff
					index = i
			node = Node()
			...
			for label in np.sort(labels):
				...
				X_playground = X[indices]

				y_playground = y[indices]


				# i=0
				# while(i < len(X_playgroundForRowDelete)):
				# 	if X_playgroundForRowDelete[i][index] != label:
				# 		X_playgroundForRowDelete = np.delete(X_playgroundForRowDelete, i, 0)
				# 		y_playgroundForRowDelete = np.delete(y_playgroundForRowDelete,i)
				# 		i = i - 1
				# 	i = i + 1
				...
				node.children.append(self.generateDecisionTree(X_playground, y_playground, y))
